...
```
<p>The summary table below shows the number of distinct breweries currently producing craft beers in the United States by state. We can see there is a heavy concentration of craft beer breweries in `r sorted.states[1]`.</p>
...
<br /> 
There is a positive linear relationship between ABV and IBU. In other words, in general, the higher the alcohol content, the more bitter the beer (and vice versa). It is estimated that the strength of this liner relationship is `r Cor.r` and r^2 = `r Cor.r2` of the variation in ABV is explained by the IBU level.
<br/>
```{r}
