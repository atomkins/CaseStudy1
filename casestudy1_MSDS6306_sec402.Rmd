...
<br /> 
There is a positive linear relationship between ABV and IBU. It is estimated that the strength of this liner relationship is `r Cor.r` and r^2 = `r Cor.r2` of the variation in ABV is explained by the IBU level.
<br/>
```{r}
...
#Conclusion
<p>In conclusion, we found that Dr. Santerre should take a trip to `r ABV.row$city`, `r ABV.row$State` and pickup the`r ABV.row$Beer_Name` from `r ABV.row$Brewery_Name` which has the highest alcohol by volume (`r ABV.row$ABV` ABV) of all beers in the list. The next stop would be `r IBU.row$City`, `r IBU.row$State` to try out`r IBU.row$Beer_Name` at `r IBU.row$Brewery_Name`. This beer has `r IBU.row$IBU` bitterness Units and is sure to delight those taste buds. 
